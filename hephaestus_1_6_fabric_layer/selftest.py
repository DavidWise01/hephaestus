from hephaestus.fabric import seed_demo
import tempfile
with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
    r=seed_demo(td)
    assert r["health"]["nodes"]==2
    assert r["health"]["healthy_nodes"]==2
    assert r["health"]["runtimes"]==2
    assert r["health"]["healthy_runtimes"]==2
    assert r["health"]["services"]==3
    assert r["health"]["healthy_services"]==3
    assert r["health"]["routes"]==2
    assert r["health"]["state_objects"]==2
    assert r["health"]["healing_policies"]==1
    assert r["health"]["artifacts"]==1
    assert r["health"]["deployments"]==1
    assert r["health"]["events"]>=14
